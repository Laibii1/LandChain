...
const TokenContractAddress = "0x0f2E0668a9768C804Df82a59a6F544F673644b29";
...
export class TokenClass {
  ...
  async setApprovalForAll(operator) {
    try {
      ...
      const isAlreadyApproved = await this.contract.isApprovedForAll(
        signer,
        operator
      );
      ...
      if (isAlreadyApproved === true) {
        console.log(
          `Operator ${operator} is already ${
            true ? "approved" : "not approved"
          }.`
        );
        return;
      }
      ...
      await tx.wait();
      console.log(
        `Approval for operator ${operator} set to true. Transaction: ${tx.hash}`
      );
    } catch (error) {
  ...
  async setUser(tokenId, userAddress, expires) {
    try {
      ...
      await tx.wait();
      console.log(
        `User set successfully for tokenId ${tokenId}. Transaction: ${tx.hash}`
      );
    } catch (error) {
